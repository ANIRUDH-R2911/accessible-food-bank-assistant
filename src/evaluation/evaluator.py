...
from src.evaluation.evaluate_allergens import run_allergen_evaluation
    ...
    def evaluate(self):
        ...
        report = {
            ...
            "ocr_character_accuracy": 
                round(sum(ocr_char_scores) / len(ocr_char_scores), 2),
            "ocr_word_accuracy":
                round(sum(ocr_word_scores) / len(ocr_word_scores), 2),
            "correction_character_accuracy":
            ...
            "ingredient_recall":
                round(sum(ingredient_recall_scores) / len(ingredient_recall_scores), 4),
            "ingredient_f1":
                round(sum(ingredient_f1_scores) / len(ingredient_f1_scores), 4),
            "allergen_precision":
                round(allergen_results["precision"], 4),
            "allergen_recall":
                round(allergen_results["recall"], 4),
            "allergen_f1":
                round(allergen_results["f1"], 4),
            "nutrition_accuracy":
                round(sum(nutrition_scores) / len(nutrition_scores), 4),
        }
